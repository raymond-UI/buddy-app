...
let TasksController = (() => {
    let _classDecorators = [(0, common_1.Controller)('tasks'), (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard)];
    ...
    var TasksController = _classThis = class {
        ...
        create(createTaskDto, req) {
            logger_1.logger.info(`Creating task for user: ${req.user.userId}`);
            return this.tasksService.create(createTaskDto, req.user.userId);
        }
        findAll(req) {
            logger_1.logger.info(`Fetching all tasks for user: ${req.user.userId}`);
            return this.tasksService.findAll(req.user.userId);
        }
        findOne(id, req) {
            logger_1.logger.info(`Fetching task ${id} for user: ${req.user.userId}`);
            return this.tasksService.findOne(id, req.user.userId);
        }
        update(id, updateTaskDto, req) {
            logger_1.logger.info(`Updating task ${id} for user: ${req.user.userId}`);
            return this.tasksService.update(id, updateTaskDto, req.user.userId);
        }
        remove(id, req) {
            logger_1.logger.info(`Deleting task ${id} for user: ${req.user.userId}`);
